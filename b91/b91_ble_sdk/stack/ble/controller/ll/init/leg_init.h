 ...
 * 			   notice that only one module can be selected between legacy initiating module
 *             and extended initiating module
 * @param	   none
 * @return     none
 */
 ...
 * @param[in]  scan_interval - recommendations from the Host on how frequently (LE_Scan_Interval)
 ...
 * @param[in]  ce_min - informative parameters providing the Controller
 *                      with the expected minimum length of the connection events.
 * @param[in]  ce_max - informative parameters providing the Controller
 *                      with the expected maximum length of the connection events.
 * @return     Status - 0x00: command succeeded; 0x01-0xFF: command failed
 */
ble_sts_t blc_ll_createConnection(scan_inter_t scanInter, scan_wind_t scanWindow, init_fp_t fp, u8 peerAdrType,
                                  u8 *peerAddr, own_addr_type_t ownAdrType, conn_inter_t conn_min,
