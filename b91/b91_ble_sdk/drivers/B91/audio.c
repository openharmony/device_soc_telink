                            ...
                            codec_wreg_mode_e wreg_mode)
{
    if (wreg_mode == MCU_WREG) {
        ...
        /* data word length ,interface master/slave selection,
        * audio interface protocol selection ,active dac audio interface
        */
        ...
        * active dac audio interface */
        ...
        audio_i2c_codec_write(addr_audio_codec_dacr_gain,
                              MASK_VAL(FLD_AUDIO_CODEC_DAC_GODR,
                                       audio_i2s_codec_config.out_digital_gain)); /* Right channel DAC digital gain */

        /* disable Headphone gain coupling, set Left channel HP amplifier gain */
        audio_i2c_codec_write(addr_audio_codec_hpl_gain, MASK_VAL(FLD_AUDIO_CODEC_HPL_LRGO, 0, FLD_AUDIO_CODEC_HPL_GOL,
                                                           ...
                                                           unsigned char match_en)
{
    ...
    switch (audio_rate) {
        ...
        case AUDIO_48K:
            ...
            } else if (match == AUDIO_RATE_LT_L0) {  // 47994.0
...
void audio_rx_dma_chain_init(dma_chn_e chn, unsigned short *in_buff, unsigned int buff_size)
{
    ...
    audio_rx_dma_add_list_element(&g_audio_rx_dma_list_cfg, &g_audio_rx_dma_list_cfg, (unsigned short *)in_buff,
                                  buff_size);
    audio_rx_dma_en();
}
...
void audio_tx_dma_chain_init(dma_chn_e chn, unsigned short *out_buff, unsigned int buff_size)
{
    ...
    audio_tx_dma_add_list_element(&g_audio_tx_dma_list_cfg, &g_audio_tx_dma_list_cfg, (unsigned short *)out_buff,
                                  buff_size);
    audio_tx_dma_en();
}
...
void audio_codec_dac_power_on(void)
{
    ...
    BM_CLR(reg_audio_codec_dac_ctr, FLD_AUDIO_CODEC_DAC_SOFT_MUTE);  // unmute
    audio_tx_dma_en();
}
